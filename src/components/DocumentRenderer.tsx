...
import { FileText, AlertCircle, RefreshCw, Image, FileIcon } from "lucide-react";
...
const DocumentRenderer = ({ document, zoom, rotation }: DocumentRendererProps) => {
  ...
  const documentName = typeof document.name === 'string' ? document.name : 'Untitled Document';
  ...
  const renderImageContent = () => {
    ...
    // Show image placeholder if no URL available
    return (
      <div className="flex items-center justify-center h-full text-center bg-gray-50">
        <div className="max-w-md">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800 mb-3">
              This image document has been uploaded successfully but preview is not available.
            </p>
      ...
      return (
        <div className="h-full overflow-auto bg-white">
          <div className="max-w-4xl mx-auto p-8">
            ...
            <div 
              className="prose prose-sm max-w-none text-gray-800 leading-relaxed whitespace-pre-wrap font-mono"
              style={{
                fontSize: `${Math.max(zoom / 100, 0.5)}em`,
                transform: `rotate(${rotation}deg)`,
                transformOrigin: 'top left'
    ...
    if (document.content) {
      return (
        <div className="h-full overflow-auto bg-white">
          <div className="max-w-4xl mx-auto p-8">
            <div className="flex items-center mb-6 pb-4 border-b">
              <FileIcon className="h-6 w-6 text-gray-600 mr-3" />
              <div>
                <h2 className="text-xl font-semibold text-gray-900">{documentName}</h2>
                <p className="text-sm text-gray-600">{document.type} • {document.size}</p>
              </div>
    ...
    return (
      <div className="flex items-center justify-center h-full text-center text-gray-500 bg-gray-50">
        <div className="max-w-md">
          ...
          <p className="text-sm mb-2">{documentName}</p>
